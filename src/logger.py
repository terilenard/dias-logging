...
class Logger(object):
    ...
    LOG_TEMPLATE = "%(asctime)s - %(levelname)-8s - Priority: %(priority)s - %(message)s"
    ...
    def __init__(self, filename, level="DEBUG") -> None:
        ...
        self._format = Formatter(Logger.LOG_TEMPLATE)
    ...
    def debug(self, msg, priority=None, do_write=True):
        return self._log(log_func=self._logger.debug, msg=msg,
            priority=priority, do_write=do_write)
    ...
    def _log(self, log_func, **kwargs):
       ...
       # msg = self._build_log(priority=prior, message=msg)

        extras = {"priority": prior}
        if do_write:
